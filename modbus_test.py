...
def check_faults(client):
    ...
    try:
        ...
        res_err = client.read_holding_registers(
            address=102, count=1, device_id=SLAVE_ID
        )
        ...
        if not res_sw.isError() and not res_err.isError():
            ...
            if is_faulted:
                ...
                if last_fault == 0:
                    print("Nota: Fault rilevato ma codice non disponibile.")
            else:
...
def continuous_monitoring():
    ...
    try:
        while True:
            ...
            res_data = client.read_holding_registers(
                address=0, count=2, device_id=SLAVE_ID
            )
            res_sw = client.read_holding_registers(
                address=50, count=1, device_id=SLAVE_ID
            )
            ...
            if not res_data.isError() and not res_sw.isError():
                ...
                print(
                    f"\rFreq: {f:6.2f}Hz | Amp: {a:5.2f}A | Stato: {status_str}   ",
                    end="",
                )
            time.sleep(0.5)
...
def read_single_register():
    """Read a single register specified by user input"""
    print("\n" + "=" * 40)
    ...
    try:
        addr_str = input("Enter register address: ")
        addr = int(addr_str)
        if addr < 0:
        ...
        res = client.read_holding_registers(address=addr, count=1, device_id=SLAVE_ID)
        if not res.isError():
            print(f"Register {addr} value: {res.registers[0]}")
        else:
            print(f"✗ Errore lettura registro {addr}: {res}")
    except ValueError:
...
if __name__ == "__main__":
    while True:
        print(
            "\n1. Test Connessione e Fault\n2. Monitoraggio Continuo\n3. Leggi Tutti i Registri\n4. Leggi Registro Singolo\n0. Esci"
        )
        scelta = input("Scelta: ")
        if scelta == "1":
            test_connection()
